...
export default function VideoEditor() {
  ...
  const [visibleModifications, setVisibleModifications] = useState<Record<string, boolean>>({})
  ...
  const handleTimeUpdate = useCallback(() => {
    ...
    // Find all active modifications at current time
    const activeEdits = changes.filter((c) => {
      if (c.type === "remove") return currentTime >= c.start && currentTime < c.end
      if (c.type === "add" || c.type === "replace") {
        return currentTime >= c.start && currentTime < c.end && c.src
      }
      return false
    })

    // Apply the first matching edit (prioritize by type: replace > add > remove)
    const prioritized = activeEdits.sort((a, b) => {
      const order = { replace: 0, add: 1, remove: 2 }
      return order[a.type] - order[b.type]
    })[0]

    if (prioritized && prioritized.type !== "remove") {
      if (activeChangeRef.current !== prioritized.id) {
        startOverlay(prioritized, {
          jumpAfterMs: prioritized.type === "replace" ? (prioritized.end - prioritized.start) * 1000 : 0,
        })
      }
    } else if (prioritized?.type === "remove") {
      // Skip removed regions
      v.currentTime = prioritized.end
    }

    // Keep cursor within selection
    setCursor(() => clamp(currentTime, start + MIN_GAP, end - MIN_GAP))
  }, [changes, start, end])
  ...
  const submitAddAuto = async () => {
    ...
    const newChange = { id: crypto.randomUUID(), type: "add" as const, start: s, end: e, src, mediaType }
    setChanges((prev) => [...prev, newChange])
    setOpenAdd(false)
    setAddFile(null)
    // Immediately play the edit at cursor if video is playing
  ...
  const submitReplaceAuto = async () => {
    ...
    const newChange = { id: crypto.randomUUID(), type: "replace" as const, start, end, src, mediaType }
    setChanges((prev) => [...prev, newChange])
    setOpenReplace(false)
    setReplaceManual(false)
    setReplaceFile(null)
    // Immediately seek to the edit if video is playing
    if (isPlaying && videoRef.current) {
  ...
  const submitRemoveAuto = () => {
    ...
    // Immediately skip removed region if currently playing through it
    if (isPlaying && videoRef.current && videoRef.current.currentTime >= start && videoRef.current.currentTime < end) {
  ...
  const activeEditOverlay = useMemo(() => {
    if (!videoRef.current) return null
    const currentTime = videoRef.current.currentTime
    const activeChange = changes.find((c) => currentTime >= c.start && currentTime < c.end)
    if (!activeChange) return null
    return activeChange.type
  }, [changes])

  return (
    <div className="flex flex-col gap-1">
      <Card className="relative overflow-hidden bg-card/60 backdrop-blur-sm glow-silver">
        <div
          className={`transition-[padding] duration-500 ease-out p-2 ${showChangesPanel ? "md:pr-[340px]" : "md:pr-0"}`}
        >
          ...
          <div className="grid gap-2 md:grid-cols-[30%_70%]">
            {/* Left: options */}
            <div className="space-y-3">
              {/* Original option */}
              <button
                className={`group relative block w-57 rounded-lg border border-border p-2 text-left transition-all hover:scale-[1.02] ${
                  previewChoice === "original" ? "ring-1 ring-[color:var(--silver)] scale-[1.02]" : "opacity-85"
                }`}
              ...
              <button
                className={`group relative block w-57 rounded-lg border border-border p-2 text-left transition-all hover:scale-[1.02] ${
                  previewChoice === "preview" ? "ring-1 ring-[color:var(--silver)] scale-[1.02]" : "opacity-85"
                }`}
            ...
            {/* Right: big chosen frame (increase height vs previous) */}
            <div className="relative">
              <div className="relative h-48 w-118 overflow-hidden rounded-lg border border-border bg-muted transition-transform hover:scale-[1.01]">
                ...
                {activeEditOverlay && (
                  <div
                    className={`pointer-events-none absolute inset-0 transition-colors duration-200 ${
                      activeEditOverlay === "add"
                        ? "bg-[color:var(--change-add)]/15"
                        : activeEditOverlay === "replace"
                          ? "bg-[color:var(--change-replace)]/15"
                          : ""
                    }`}
                  />
                )}
        ...
        <aside
          className={`absolute right-0 top-0 h-full w-[320px] border-l border-border bg-card/80 backdrop-blur-md transition-transform duration-500 ease-out ${
            showChangesPanel ? "translate-x-0" : "translate-x-full"
          }`}
          style={{ zIndex: 60 }}
        >
          <div className="p-3">
        ...
        <div className="p-2">
          ...
          <div className="relative select-none">
            ...
            >
                ...
                return c.type === "remove" ? (
                  <div
                    ...
                    style={{
                      left: `${left}%`,
                      width: `${width}%`,
                      backgroundColor: "var(--change-remove)",
                      opacity: 0.6,
                    }}
              ...
              {/* Cursor: taller white dotted line */}
              <div
                role="slider"
                aria-label="Cursor"
                aria-valuemin={0}
            ...
            {/* Cursor tools anchored to cursor with longer hover linger and high z-index */}
            <div
            ...
            >
              <div className="flex items-center gap-2">
                {/* Play/Pause button controlling preview from cursor */}
                <button
              ...
              {/* Hover tools popover */}
              <div
                className={`absolute left-1/2 mt-2 -translate-x-1/2 rounded-xl border border-border bg-popover px-3 py-2 shadow-xl backdrop-blur-md transition-opacity ${
...
function SwapIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="none" {...props}>
      <path d="M7 7h11M7 7l3-3M7 7l3 3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
      <path d="M17 17H6m11 0-3-3m3 3-3 3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  )
}
function TrashIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="none" {...props}>
      <path
        d="M4 7h16M9 7l1-2h4l1 2M8 7v12a2 2 0 0 0 2 2h4a2 2 0 0 0 2-2V7"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
      />
    ...
    <li className="rounded-lg border border-border">
      ...
      {open && (
        <div className="space-y-2 border-t border-border p-2 text-xs">
          <div className="rounded-md bg-muted/40 p-2 text-muted-foreground">Preview not available in demo.</div>
          {entry.type === "replace" && (
